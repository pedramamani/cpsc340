...
def q2_2():
    ...
    for lammy in [0.01, 0.1, 1, 10]:
        fun_obj = LogisticRegressionLoss()
        optimizer = GradientDescentLineSearchProxL1(lammy=lammy)
        model = linear_models.LogRegClassifier(fun_obj, optimizer)
        model.fit(X, y)

        print(f"\nLambda = {lammy}:")
        train_err = utils.classification_error(model.predict(X), y)
        print(f"Training error: {train_err:.5f}")

        val_err = utils.classification_error(model.predict(X_valid), y_valid)
        print(f"Validation error: {val_err:.3f}")

        print(f"Number of used features: {np.sum(model.w != 0)}")
        print(f"Number of function evaluations: {optimizer.num_evals}")
...
def q3_5():
    ...
    oneVsAll = LogisticRegression(multi_class='ovr', penalty='none', fit_intercept=False)
    softMax = LogisticRegression(multi_class='multinomial', penalty='none', fit_intercept=False)
    oneVsAll.fit(X, y)
    softMax.fit(X, y)

    train_err = utils.classification_error(oneVsAll.predict(X), y)
    print(f"one-vs-all train: {train_err:.3f}")
    val_err = utils.classification_error(oneVsAll.predict(X_valid), y_valid)
    print(f"one-vs-all validation: {val_err:.3f}")

    train_err = utils.classification_error(softMax.predict(X), y)
    print(f"softmax train: {train_err:.3f}")
    val_err = utils.classification_error(softMax.predict(X_valid), y_valid)
    print(f"softmax validation: {val_err:.3f}")

    print(softMax.predict(X_valid))
