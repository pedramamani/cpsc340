...
class DecisionTree:
    ...
    def fit(self, X, y):
        ...
        # Fit a decision tree to each split, decreasing maximum depth by one
        ...
        self.stump_model = stump_model
        self.submodel_yes = DecisionTree(self.max_depth - 1, stump_class=self.stump_class)
        self.submodel_yes.fit(X[yes], y[yes])
        self.submodel_no = DecisionTree(self.max_depth - 1, stump_class=self.stump_class)
    ...
    def predict(self, X):
        ...
        # the case with depth=one, just a single stump.
