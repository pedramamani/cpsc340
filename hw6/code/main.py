...
def q2_1():
    ...
    print(f"Non-missing in Y_train: {np.sum(~np.isnan(Y_train))}")
    print(f"Non-missing in Y_valid: {np.sum(~np.isnan(Y_valid))}")
    print(f"Non-missing in Y_train + Y_valid: {np.sum(~np.isnan(Y_train + Y_valid))}")
...
def q2_3():
    ...
    lammyZ = 10
    lammyY = 10
    fun_obj_w = CollaborativeFilteringWLoss(lammyZ=lammyZ, lammyW=lammyY)
    fun_obj_z = CollaborativeFilteringZLoss(lammyZ=lammyZ, lammyW=lammyY)
...
def q3():
    ...
    learning_rate_getter = ConsantLR(1e-3)
...
def q3_2():
    ...
    fun_obj = MLPLogisticRegressionLossL2(encoder, predictor, lammy)

    # Choose optimization strategy
    child_optimizer = GradientDescent()
    learning_rate_getter = ConstantLR(lr)
    optimizer = StochasticGradient(
        child_optimizer, learning_rate_getter, bs, max_evals=n_evals
    )
