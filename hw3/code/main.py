...
def q2_1():
    ...
    utils.test_and_plot(
        model, X, y, title="Weighted Least Squares", filename="weighted_least_squares.pdf"
    )
...
def q2_4_2():
    ...
    optimizer = OptimizerGradientDescentLineSearch(max_evals=100, verbose=False)
    model_ls = linear_models.LinearModelGradientDescent(fun_obj, optimizer)
    model_ls.fit(X, y)

    p = myplot.Plot()
    p.line(np.arange(0, 101), model.fs)
    p.line(np.arange(0, 101), np.pad(model_ls.fs, (0, 101 - len(model_ls.fs)), 'minimum'))
    p.show(legend=['OptimizerGradientDescent', 'OptimizerGradientDescentLineSearch'],
           title='Gradient Descent Learning Curves',
           xlabel='Iteration #', ylabel='Objective function value', grid=True)
